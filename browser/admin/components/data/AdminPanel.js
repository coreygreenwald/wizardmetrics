        ...
        <div className="admin-panel-funnel">
          {
            info && info.length ? mostCommonJourney(info).map(({actionData, percent, occurrences, time, totalCount }) => {
              return (
                <div style={{width: `${((Number(occurrences) / completedJourneys) * 50) + 50}%`, height: '150px', backgroundColor: colorChooser(occurrences, completedJourneys)}} className="admin-panel-funnel-item">
